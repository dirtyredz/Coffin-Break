// Copied verbatim from mods/ChestLabels/src/ChestLabels/GameFonts.cs, namespace aside.
...
namespace CoffinBreak
{
    ...
    internal static class GameFonts
    {
        ...
        internal static TMP_FontAsset HeavyFont
        {
            get
            {
                ...
                try
                {
                    ...
                    CoffinBreakPlugin.Log.LogInfo(
                        $"Heavy font: {(heavyFont == null ? "Gelica-Black not loaded, using Bold" : heavyFont.name)}");
                }
                catch (Exception e)
                {
                    CoffinBreakPlugin.Log.LogWarning($"Heavy font lookup failed: {e.Message}");
                }
        ...
        private static void Search()
        {
            ...
            {
                CoffinBreakPlugin.Log.LogWarning($"Font lookup failed, falling back to TMP default: {e.Message}");
            }
        }
    }
}
